...
const getCoordsFromLocation = async () => {
	...
	const settings = //{ location, port, locationiq_api_key }
		await readSettings(["location", "port"]);
...
export const getCoords = async () => {
	const coords = await readSettings(["lat", "lon"]);
...
export async function getCoordsExternal({ location, apiKey }) {
	const url = `https://us1.locationiq.com/v1/search?key=${apiKey}&q=${location}&format=json`;
